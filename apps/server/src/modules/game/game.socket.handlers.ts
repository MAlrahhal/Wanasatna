...
export function registerGameShellInitHandler(io: Server, socket: Socket): void {
  socket.on(GAME_SHELL_INIT_EVENT, async (payload: unknown, callback) => {
...
export function registerGameShellSyncHandler(io: Server, socket: Socket): void {
  socket.on(GAME_SHELL_SYNC_EVENT, async (_payload: unknown, callback) => {
    ...
    try {
      ...
      sendGameResponse(callback, response.success ? { success: true, data: { state } } : response);
    } catch {
      sendGameInternalError(callback);
    }
  });
...
export function registerGameShellSetReadyHandler(io: Server, socket: Socket): void {
  socket.on(GAME_SHELL_SET_READY_EVENT, async (payload: unknown, callback) => {
    const contextError = getGameSocketContext(socket);
    ...
    try {
      const response = await setGameShellReady(roomId!, playerId!, validation.data.isReady);
...
export function registerGameShellStartFromLobbyHandler(io: Server, socket: Socket): void {
  socket.on(GAME_SHELL_START_FROM_LOBBY_EVENT, async (payload: unknown, callback) => {
    const contextError = getGameSocketContext(socket);
    ...
    try {
      ...
      const response = await startGameShellFromLobby(roomId!, playerId!, validation.data.gameId);
